...
def default_argument_parser():
    ...
    parser.add_argument("--teacher-config-file", default="configs_teacher/Market1501/myconfig.yml", metavar="FILE", help="path to teacher config file")
    parser.add_argument(
        "--resume",
        action="store_true",
...
class DefaultPredictor:
    ...
    def __init__(self, cfg):
        ...
        self.model = build_model(self.cfg,self.cfg)
        self.model.eval()
...
class DefaultTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg,teacher_cfg):
        """
        Args:
            cfg (CfgNode):
        """
        ...
        teacher_cfg = self.auto_scale_hyperparams(teacher_cfg, data_loader)
        ...
        optimizer_teacher= self.build_optimizer(teacher_cfg,model_teacher)
        ...
        super().__init__(model, model_teacher, data_loader, optimizer,optimizer_teacher)

        self.scheduler = self.build_lr_scheduler(cfg, optimizer)

        self.scheduler_teacher = self.build_lr_scheduler(teacher_cfg, optimizer_teacher)
        ...
        self.checkpointer_teacher = Checkpointer(
            model_teacher,
            teacher_cfg.OUTPUT_DIR,
            save_to_disk=comm.is_main_process(),
            optimizer=optimizer_teacher,
            scheduler=self.scheduler_teacher,
        )
        print("checkpoints is ok")
        self.start_iter = 0
        ...
        ret,ret_new=self.build_hooks()
        self.register_hooks(ret,ret_new)
    ...
    def build_hooks(self):
        ...
        teacher_cfg = self.teacher_cfg.clone()
        teacher_cfg.defrost()
        teacher_cfg.DATALOADER.NUM_WORKERS = 0  # save some memory and time for PreciseBN
        teacher_cfg.DATASETS.NAMES = tuple([teacher_cfg.TEST.PRECISE_BN.DATASET])  # set dataset name for PreciseBN
        ...
        ret_new = [
            hooks.IterationTimer(),
            hooks.LRScheduler(self.optimizer_teacher, self.scheduler_teacher),
        ]
        ...
        if cfg.TEST.PRECISE_BN.ENABLED and hooks.get_bn_modules(self.model):
            ...
            ret.append(hooks.PreciseBN(
                ...
                self.model_teacher,
                # Build a new data loader to not affect training
                self.build_train_loader(teacher_cfg),
                cfg.TEST.PRECISE_BN.NUM_ITER,
            ))
        ...
        if teacher_cfg.SOLVER.SWA.ENABLED:
            ret_new.append(
                hooks.SWA(
                    teacher_cfg.SOLVER.MAX_ITER,
                    teacher_cfg.SOLVER.SWA.PERIOD,
                    teacher_cfg.SOLVER.SWA.LR_FACTOR,
                    teacher_cfg.SOLVER.SWA.ETA_MIN_LR,
                    teacher_cfg.SOLVER.SWA.LR_SCHED,
                )
            )

        if teacher_cfg.TEST.PRECISE_BN.ENABLED and hooks.get_bn_modules(self.model_teacher):
            logger.info("Prepare precise BN dataset")
            ret_new.append(hooks.PreciseBN(
                # Run at the same freq as (but before) evaluation.
                self.model_teacher,
                # Build a new data loader to not affect training
                self.build_train_loader(teacher_cfg),
                teacher_cfg.TEST.PRECISE_BN.NUM_ITER,
            ))
            ret_new.append(hooks.PreciseBN(
                # Run at the same freq as (but before) evaluation.
                self.model_teacher,
        ...
        if teacher_cfg.MODEL.FREEZE_LAYERS != [''] and teacher_cfg.SOLVER.FREEZE_ITERS > 0:
            freeze_layers = ",".join(teacher_cfg.MODEL.FREEZE_LAYERS)
            logger.info(f'Freeze layer group "{freeze_layers}" training for {teacher_cfg.SOLVER.FREEZE_ITERS:d} iterations')
            ret_new.append(hooks.FreezeLayer(
                self.model_teacher,
                self.optimizer_teacher,
                teacher_cfg.MODEL.FREEZE_LAYERS,
                teacher_cfg.SOLVER.FREEZE_ITERS,
            ))
        ...
        if comm.is_main_process():
            ...
            ret_new.append(hooks.PeriodicCheckpointer(self.checkpointer_teacher, teacher_cfg.SOLVER.CHECKPOINT_PERIOD))

        def test_and_save_results():
            self._last_eval_results = self.test(self.cfg, self.model)
            return self._last_eval_results
        def test_and_save_results_new():
            self._last_eval_results_new = self.test(self.teacher_cfg, self.model_teacher)
            return self._last_eval_results_new
        # Do evaluation after checkpointer, because then if it fails,
        # we can use the saved checkpoint to debug.
        ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, test_and_save_results))
        ret_new.append(hooks.EvalHook(teacher_cfg.TEST.EVAL_PERIOD, test_and_save_results_new))
        
        if comm.is_main_process():
            ret.append(hooks.PeriodicWriter(self.build_writers(), 200))
            ret_new.append(hooks.PeriodicWriter(self.build_writers_new(), 200))
        return ret,ret_new
    ...
    def build_writers_new(self):
        return [
            # It may not always print what you want to see, since it prints "common" metrics only.
            CommonMetricPrinter(self.max_iter),
            JSONWriter(os.path.join(self.teacher_cfg.OUTPUT_DIR, "metrics.json")),
            TensorboardXWriter(self.teacher_cfg.OUTPUT_DIR),
        ]
    ...
    def train(self):
        ...
        if comm.is_main_process():
            ...
            return self._last_eval_results,self._last_eval_results_new
