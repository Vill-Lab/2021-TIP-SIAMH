...
def hard_example_mining(dist_mat, is_pos, is_neg):
    ...
    print("note2:",dist_mat[is_pos].contiguous().shape)
    print("note1:",dist_mat[is_neg].contiguous().shape)
    ...
    print("yes:",dist_mat[is_pos].contiguous().view(N, -1).shape)
    # shape [N]
...
def weighted_example_mining(dist_mat, is_pos, is_neg):
    ...
    return dist_ap, dist_an
def hard_mine_loss(inputs,targets):
    n = inputs.size(0)
...
class TripletLoss(object):
    ...
    def __call__(self, embedding, targets):
        ...
        if self._hard_mining:
            dist_ap, dist_an = hard_mine_loss(embedding,targets)#hard_example_mining(dist_mat, is_pos, is_neg)
        else:
